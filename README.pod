...
Netsync - network/database synchronization utility/library
...
Note: If installation is skipped, netsync may be manually invoked as follows:
...
Note: If neither DNS (-D) nor Nodes are specified,
...
Note: The settings above are not necessary if only the Nodes parameter is used.
...
Note: If the Update (-u) option is used,
...
Note: The settings above are not necessary if only the Database (-d) option is used.
...
Z<>
 ...
 $ netsync -Dqp1
 $ netsync -qm "(sw|hub)[0-9]+" /var/cache/netsync/active.txt
...
It may subsequently be passed as the Nodes parameter to skip inactive nodes.
 ...
 > discovering (using DBMS)...  480 nodes (10 inactive), 500 devices
 $ netsync /var/cache/netsync/active.txt
 > configuring (using /etc/netsync/netsync.ini)...
 > discovering (using /var/cache/netsync/active.txt)...  480 nodes, 500 devices
 > identifying (using DBMS)...  498 recognized (4 conflicts)

Note: Since the goal of Probe Level 1 is to save time later,
...
It may be passed as [nodes] in subsequent runs to skip synchronization.
 ...
 > discovering (using DNS)...  480 nodes (10 inactive), 500 devices
 > identifying (using DBMS)...  498 recognized (4 conflicts)
 > Do you want to resolve conflicts now? [y/n] n
 $ netsync -d /var/cache/netsync/synced.csv /var/cache/netsync/active.txt
 > configuring (using /etc/netsync/netsync.ini)... done
 > discovering (using /var/cache/netsync/active.txt)...  480 nodes, 500 devices
 > identifying (using /var/cache/netsync/synced.csv)...  500 recognized

Note: All unidentified hardware will be present in Probe2Cache.
